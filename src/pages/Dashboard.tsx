    ...
    <div className="flex min-h-screen bg-background">
      ...
      <aside className="hidden w-64 flex-col border-r border-border bg-navy-deep lg:flex">
        ...
        <div className="border-t border-border p-4">
          <Link to="/">
            <button className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-muted-foreground transition-colors hover:bg-navy-elevated hover:text-foreground">
              <LogOut size={18} />
              Sign Out
            </button>
          </Link>
        ...
        {sidebarOpen && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-40 bg-navy-deep/80 lg:hidden"
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-50 flex h-full w-64 flex-col border-r border-border bg-navy-deep lg:hidden"
            >
            ...
            <div className="space-y-4">
              {recentActivity.map((a, i) => (
                <div key={i} className="flex items-start gap-3">
                  <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-steel/10 text-steel">
                    <a.icon size={16} />
                  </div>
